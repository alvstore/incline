        ...
        {filterRule && (
          <div className="mt-2 flex items-center gap-2 text-xs">
            ...
            <Badge className="bg-primary/15 text-primary gap-1 hover:bg-primary/15">
              {filterRule.name}
              <button onClick={onClearFilter} aria-label="Clear filter" className="ml-1 hover:text-primary">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          </div>
              ...
              return (
                <li key={r.id} className="py-2.5 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-foreground flex-1 truncate text-xs">
                  ...
                  <div className="text-xs text-muted-foreground pl-1 flex items-center gap-2 flex-wrap">
                    <span>{r.dispatched_count} dispatched</span>
                    {r.error_message && (
                      <span className="text-destructive truncate max-w-full" title={r.error_message}>
                        {r.error_message}
                      </span>
                    )}
                  </div>
