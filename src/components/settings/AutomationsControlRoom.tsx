...
export function AutomationsControlRoom() {
  ...
  const kpis: Array<{ label: string; value: string | number; icon: typeof Activity; tone: string; trend: string | null }> = [
    ...
    },
    {
      ...
      icon: AlertTriangle,
      tone: stats.failures24 > 0 ? 'bg-destructive/10 text-destructive' : 'bg-muted text-muted-foreground',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-start gap-4">
          <div className="bg-gradient-to-br from-primary to-primary text-primary-foreground p-3 rounded-2xl shadow-lg shadow-primary/20">
            <Bot className="h-6 w-6" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-foreground">Automation Brain</h2>
          ...
          <Card key={k.label} className="rounded-2xl shadow-lg shadow/50 border-0">
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                {k.trend && <p className="text-xs text-muted-foreground mt-1">{k.trend}</p>}
              </div>
        ...
        <div className="space-y-4 min-w-0">
          ...
          <Card className="rounded-2xl shadow-lg shadow/50 border-0">
            <CardContent className="p-3 flex items-center gap-2 flex-wrap">
              ...
              <label className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-border cursor-pointer hover:bg-muted">
                <Switch checked={failingOnly} onCheckedChange={setFailingOnly} aria-label="Failing only" />
                <span className="text-sm text-foreground">Failing only</span>
              </label>
              <label className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-border cursor-pointer hover:bg-muted">
                <Switch checked={aiOnly} onCheckedChange={setAiOnly} aria-label="AI only" />
                <span className="text-sm text-foreground">AI only</span>
              </label>
            </CardContent>
          </Card>
                    ...
                    <CollapsibleTrigger asChild>
                      <CardHeader className="pb-3 cursor-pointer hover:bg-muted/50 rounded-t-2xl">
                        <CardTitle className="flex items-center gap-2 text-base">
                          ...
                          <span className="text-foreground text-sm font-normal">{list.length} automation{list.length === 1 ? '' : 's'}</span>
                          {failing > 0 && (
                            <Badge className="bg-destructive/15 text-destructive gap-1 ml-auto hover:bg-destructive/15">
