...
const PROVIDER_SCHEMAS: Record<string, ProviderFieldDef[]> = {
  ...
  payment_gateway_razorpay: [
    ...
    { key: 'webhook_secret', label: 'Webhook Secret', placeholder: 'Enter Razorpay Webhook Secret', type: 'password', section: 'credentials' },
    {
      ...
      label: 'Charge convenience fee on online payments',
      placeholder: '',
      type: 'select',
      section: 'config',
      options: [{ value: 'false', label: 'Off' }, { value: 'true', label: 'On' }],
    },
    { key: 'convenience_fee_percent', label: 'Convenience fee %', placeholder: 'e.g. 2.36 (Razorpay card rate)', type: 'text', section: 'config' },
    { key: 'convenience_fee_fixed', label: 'Flat add-on (₹)', placeholder: 'e.g. 0', type: 'text', section: 'config' },
    { key: 'convenience_fee_gst_percent', label: 'GST on fee %', placeholder: 'Default 18', type: 'text', section: 'config' },
    { key: 'convenience_fee_cap', label: 'Maximum fee (₹, optional)', placeholder: 'Leave blank for no cap', type: 'text', section: 'config' },
    { key: 'convenience_fee_label', label: 'Label shown to member', placeholder: 'Online payment convenience fee', type: 'text', section: 'config' },
  ],
  ...
  whatsapp_meta_cloud: [
    ...
    { key: 'app_id', label: 'Meta App ID', placeholder: 'From Meta App Dashboard → Settings → Basic (required for image/video/document template headers)', type: 'text', section: 'config' },
