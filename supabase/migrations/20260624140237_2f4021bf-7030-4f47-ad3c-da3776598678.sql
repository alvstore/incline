...
-- 1. Rewrite purchase_member_membership: respect future start_date (keep pending until start day)
...
BEGIN
  ...
  v_is_advance := p_start_date > current_date;
  ...
  -- Activation rules:
  --   * Future start_date  -> always stay 'pending' (advance booking). Activated by cron on start day.
  --   * Start today/past   -> activate when fully paid OR any payment received with a non-overdue due date.
  v_new_status := CASE
    WHEN v_is_advance THEN 'pending'::public.membership_status
  ...
  PERFORM public.log_member_lifecycle_event(
    ...
    v_membership.id,
    CASE WHEN v_is_advance THEN 'membership_scheduled' ELSE 'membership_purchased' END,
    NULL, v_membership.status::text,
    p_payment_source, p_notes, p_idempotency_key,
    jsonb_build_object('invoice_id', v_invoice.id, 'plan_id', p_plan_id,
                       'remaining_amount', v_remaining_amount,
                       'scheduled_start', p_start_date, 'is_advance_booking', v_is_advance)
  );

  -- Do not evaluate hardware access for advance bookings; existing access (if any) is untouched.
  ...
  RETURN jsonb_build_object(
    ...
    'is_advance_booking', v_is_advance, 'scheduled_start', p_start_date
  );
END;
$function$;

-- 2. Daily job: flip pending -> active on start_date
...
BEGIN
  ...
  LOOP
    ...
    -- Activate any pre-staged locker for this scheduled membership
    ...
    PERFORM public.log_member_lifecycle_event(
      ...
      'cron', 'Advance booking activated on start date', NULL,
      jsonb_build_object('source', 'activate_scheduled_memberships')
    );
    PERFORM public.evaluate_member_access_state(v_row.member_id, NULL, 'Scheduled membership activated', true);

    v_count := v_count + 1;
  END LOOP;
  RETURN jsonb_build_object('activated', v_count, 'ran_at', now());
END;
$$;

GRANT EXECUTE ON FUNCTION public.activate_scheduled_memberships() TO service_role;

-- 3. Schedule via pg_cron (00:05 IST = 18:35 UTC). Drop prior schedule if any.
DO $$
BEGIN
  ...
  PERFORM cron.schedule(
    ...
    '35 18 * * *',
    $cron$ SELECT public.activate_scheduled_memberships(); $cron$
  );
EXCEPTION WHEN OTHERS THEN
  -- pg_cron not installed in some envs; ignore
  NULL;
END$$;

-- 4. Backfill Love Kumar Paliwal (INC-26-0004): start 2026-07-27, end +364 days, status pending
UPDATE public.memberships m
   SET start_date     = DATE '2026-07-27',
       end_date       = DATE '2026-07-27' + (mp.duration_days - 1),
       original_end_date = DATE '2026-07-27' + (mp.duration_days - 1),
       status         = 'pending'::public.membership_status,
       updated_at     = now()
  FROM public.members mb, public.membership_plans mp
 WHERE mb.member_code = 'INC-26-0004'
   AND m.member_id = mb.id
   AND m.plan_id = mp.id
   AND m.status = 'active'::public.membership_status
   AND m.start_date = DATE '2026-06-24';
