-- Redefine create_system_notification to match what the trigger expects
...
-- Ensure create_pos_sale supports the frontend call
...
BEGIN
  ...
  INSERT INTO public.pos_sales (branch_id, member_id, items, total_amount, payment_method, sold_by, customer_name, customer_phone, customer_email, payment_status)
  ...
  IF NOT v_is_awaiting AND v_remainder > 0 THEN
    PERFORM public.settle_payment(
      ...
      p_notes := v_notes, p_received_by := p_sold_by, p_payment_source := 'pos_sale',
