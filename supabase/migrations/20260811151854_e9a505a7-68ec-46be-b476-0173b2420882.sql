CREATE OR REPLACE FUNCTION public.purchase_member_membership(p_member_id uuid, p_plan_id uuid, p_branch_id uuid, p_start_date date, p_discount_amount numeric DEFAULT 0, p_discount_reason text DEFAULT NULL::text, p_include_gst boolean DEFAULT false, p_gst_rate numeric DEFAULT 0, p_payment_method text DEFAULT 'cash'::text, p_amount_paying numeric DEFAULT 0, p_payment_due_date date DEFAULT NULL::date, p_send_reminders boolean DEFAULT true, p_payment_source text DEFAULT 'manual'::text, p_idempotency_key text DEFAULT NULL::text, p_assign_locker_id uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text, p_received_by uuid DEFAULT auth.uid(), p_transaction_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF COALESCE(p_amount_paying, 0) > 0 THEN
    v_payment_result := public.settle_payment(
      ...
      NULLIF(btrim(COALESCE(p_transaction_id, '')), ''), p_notes, p_received_by, NULL, p_payment_source, p_idempotency_key, NULL, NULL,
