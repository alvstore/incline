CREATE OR REPLACE FUNCTION public._purchase_pt_package_impl(
  ...
  _price_paid numeric, _idempotency_key text, _gst_rate numeric,
  _payment_method text, _payment_source text, _start_date date,
  _received_by uuid, _amount_paid numeric DEFAULT NULL,
  _due_date date DEFAULT NULL, _transaction_id text DEFAULT NULL,
...
BEGIN
  ...
  ELSIF _payment_source = 'in_person' AND _collect = 0 THEN
    -- Credit sale: nothing collected today, but the sale is committed.
    -- Keep the package active with the balance tracked on the invoice, and
    -- clear the pending-reversal window so the reaper never cancels it.
    UPDATE public.member_pt_packages
    SET status = 'active'::pt_package_status,
