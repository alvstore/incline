...
CREATE POLICY contacts_delete_staff ON public.contacts
FOR DELETE
USING (
...
CREATE OR REPLACE FUNCTION public.resolve_campaign_audience(p_branch_id uuid, p_filter jsonb)
 RETURNS TABLE(source_type text, source_ref_id uuid, full_name text, phone text, email text, contact_id uuid)
 LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_kind IN ('members','mixed','members_and_staff') THEN
    RETURN QUERY
  ...
  IF v_kind IN ('staff','mixed','members_and_staff') THEN
    RETURN QUERY
