-- 1) Remove the dead duplicate overload created with a different parameter order
DROP FUNCTION IF EXISTS public._purchase_pt_package_impl(uuid,uuid,uuid,uuid,numeric,text,numeric,text,text,date,uuid,numeric,date,text,text);

-- 2) Notification type normalization: 'task' is not a valid notifications.type
CREATE OR REPLACE FUNCTION public.create_system_notification(p_user_id uuid, p_title text, p_message text, p_type text DEFAULT 'info'::text, p_linked_entity_id text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  v_category := p_type;
  -- notifications.type only allows info/success/warning/error/reminder
  v_type := CASE
    WHEN p_type IN ('info','success','warning','error','reminder') THEN p_type
    ELSE 'info'
  END;
  ...
  INSERT INTO public.notifications (
    user_id, branch_id, title, message, type, category, action_url, metadata
  )
  VALUES (
    p_user_id, v_branch_id, p_title, p_message, v_type, v_category, v_action_url,
    jsonb_build_object(
...
-- 3) PT purchase: activate credit sales (zero collected) instead of leaving pending_payment
...
BEGIN
  ...
  ELSIF _payment_source = 'in_person' AND _collect = 0 THEN
    -- Credit sale: package is assigned now, full amount stays due on the invoice
    UPDATE public.member_pt_packages
    SET status = 'active'::pt_package_status,
