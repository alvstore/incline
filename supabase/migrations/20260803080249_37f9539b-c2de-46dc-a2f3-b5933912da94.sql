DROP FUNCTION IF EXISTS public.correct_invoice(uuid, numeric, numeric, numeric, numeric, text, text, text);
...
BEGIN
  ...
  v_rate := coalesce(p_gst_rate, v_inv.gst_rate);
  IF p_new_tax = 0 THEN
    v_rate := 0;
  END IF;
  ...
  UPDATE public.invoices
     SET subtotal = p_new_subtotal,
         ...
         gst_rate = v_rate,
         is_gst_invoice = (coalesce(v_rate,0) > 0),
         status = (CASE
